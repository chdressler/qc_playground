...
def nuc_pp(type1, type2, exp1, exp2, r1, r2, r3):
    ...
    if i == j:
        ...
        nuc = (
            ...
            * s2(t)
            + (2 - (4 * exp1 * exp2 * (r1[i] - r2[i]) * (r1[j] - r2[j])) / nu) * s1(t)
        )
    else:
        nuc = (
            nu * rr[i] * rr[j] * s3(t)
            + (2 * exp1 * (r1[j] - r2[j]) * rr[i] - 2 * exp2 * (r1[i] - r2[i]) * rr[j])
            * s2(t)
            + (-1 * (4 * exp1 * exp2 * (r1[i] - r2[i]) * (r1[j] - r2[j])) / nu) * s1(t)
        )
...
def nuc_ps(type1, type2, exp1, exp2, r1, r2, r3):
    ...
    nuc = (
        ...
        * np.pi ** 1.5
        / (2 * nu)
        * (rr[i] * s2(t) - (2 * exp2 * (r1[i] - r2[i])) / nu * s1(t))
    )
    nuc *= (128 * exp1 ** 5 / np.pi ** 3) ** 0.25
    nuc *= (2.0 * exp2 / np.pi) ** 0.75
    return nuc
