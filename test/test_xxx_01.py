...
def test_form_g_mat():
    ...
    orca_dens = np.loadtxt(
        pkg_resources.resource_filename("bombanitio", "data/dens_xxx_01")
    )
    orca_fock = np.loadtxt(
        pkg_resources.resource_filename("bombanitio", "data/fock_xxx_01")
    )
    orca_h = np.loadtxt(pkg_resources.resource_filename("bombanitio", "data/h_xxx_01"))
    ...
    coord, atom = read_xyz.easy_read(
        pkg_resources.resource_filename("bombanitio", "data/coord_xxx_01.xyz"),
        None,
        False,
        False,
    )
